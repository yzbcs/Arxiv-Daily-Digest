...
import execjs
from fetchers.xhs_cookie_util import trans_cookies

_project_root = Path(__file__).parent.parent
# execjs 临时文件模式在 /tmp 执行 JS，Node 找不到 node_modules，需手动指定路径
os.environ["NODE_PATH"] = str(_project_root / "node_modules")

_node_rt = execjs.get()
_node_rt._tempfile = True  # 绕过 Node.js v22+ 的 stdin TS parser

_static = Path(__file__).parent.parent / "static"
js = _node_rt.compile((_static / "xhs_xs_xsc_56.js").read_text(encoding="utf-8"))
# xhs_xray.js 在 Node v25 有 CJS 兼容问题，x-xray-traceid 为非必需，直接返回空字符串
